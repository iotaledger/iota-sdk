---
description: Easily integrate with your exchange, custody solution, or product using the iota-sdk library.
...
import generate_mnemonic from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/0-generate-mnemonic.js';
import create_account from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/1-create-account.js';
import generate_address from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/2-generate-address.js';
import check_balance from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/3-check-balance.js';
import listen_events from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/4-listen-events.js';
import send_amount from '!!raw-loader!./../../../../sdk/src/wallet/bindings/nodejs/examples/exchange/5-send-amount.js';
...
If you are not familiar with the iota-sdk library, you can find more information in the [documentation](../welcome.md).

:::

For changes since the wallet.rs version for chrysalis (version 1.x) see the [changelog](https://github.com/iotaledger/iota-sdk/blob/develop/sdk/src/wallet/bindings/nodejs/CHANGELOG.md).

You can use the following examples as a guide to implementing *the multi-account approach* using the `NodeJS` binding:

1. [Set up the iota-sdk library](#1-set-up-the-wallet-library).
...
If you are looking for other languages, please read the [iota-sdk library overview](./../explanations/library_overview.md).

:::

Since all `iota-sdk` bindings are based on core principles provided by the `iota-sdk` library, the outlined approach is very similar regardless of the programming language you choose.

### 1. Set Up the iota-sdk Library

First, you should install the components that are needed to use `iota-sdk` and the binding of your choice; it may vary a bit from language to language. In the case of the [NodeJs binding](./../getting_started/nodejs.mdx), it is straightforward since it is distributed via the `npm` package manager.
...
You can now add your `STRONGHOLD_PASSWORD` and `MNEMONIC` to the `.env` file. 

```bash
STRONGHOLD_PASSWORD="here is your super secure password"
MNEMONIC="here is your super secure 24 word mnemonic, it's only needed here the first time"
```

After you have updated the `.env` file, you can initialize the `AccountManager` instance with a secret manager(like [`Stronghold`](https://wiki.iota.org/stronghold.rs/welcome) or LedgerNano) and client options.
...
The wallet is a stateful library. This means it caches all relevant information in storage to provide performance benefits while dealing with, potentially, many accounts and addresses.
<CodeBlock className="language-javascript">
  {generate_address}
</CodeBlock>
...
* Internal addresses (`internal=true`) are called `change` addresses and can be used to send the excess funds to them by providing `RemainderValueStrategy::ChangeAddress` to the TransactionOptions.
...
The `iota-sdk` library supports several events for listening. A provided callback is triggered as soon as an event occurs (which usually happens during syncing).
...
The `account.sendAmount()` function returns a `transaction` with its id. The `blockId` can be used later for checking a confirmation status. You can obtain individual transactions related to the given account using the `account.transactions()` function.

:::note Dust Protection

When sending tokens, you should consider the [dust protection based on byte cost](https://wiki.iota.org/shimmer/tips/tips/TIP-0019).
Amounts below the minimum storage deposit can only be sent with a storage deposit return unlock condition, which require the receiver to send the storage deposit back when consuming the output.
In the shimmer network the minimum amount for a basic output is 42600.
