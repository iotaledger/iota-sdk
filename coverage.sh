...
RUSTFLAGS="-C instrument-coverage" LLVM_PROFILE_FILE="iota-%m.profraw" cargo +nightly test --profile ci --tests --package iota-sdk --all-features -- --include-ignored
...
for file in \
  $( \
    RUSTFLAGS="-C instrument-coverage" \
      cargo +nightly test --profile ci --tests --package iota-sdk --all-features --no-run --message-format=json -- --include-ignored \
