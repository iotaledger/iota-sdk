...
async fn main() -> Result<()> {
    ...
    let basic_output_builder = BasicOutputBuilder::new_with_minimum_amount(rent_parameters)
        .add_unlock_condition(AddressUnlockCondition::new(address.clone()));
    let account_output_builder = AccountOutputBuilder::new_with_minimum_amount(rent_parameters, AccountId::null());
    let foundry_output_builder = FoundryOutputBuilder::new_with_minimum_amount(rent_parameters, 1, token_scheme);
