...
async fn main() -> Result<()> {
    ...
    let (wallet, secret_manager) = create_wallet().await?;

    // Change to `true` to print the full balance report
    sync_print_balance(&wallet, &secret_manager, false).await?;
...
async fn sync_print_balance(wallet: &Wallet, secret_manager: &MnemonicSecretManager, full_report: bool) -> Result<()> {
