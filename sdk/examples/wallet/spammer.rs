...
use iota_sdk::{
    client::{
        ...
        secret::{mnemonic::MnemonicSecretManager, SecretManage},
    },
...
async fn main() -> Result<()> {
    ...
    ensure_enough_funds(&wallet, &secret_manager, &recv_address).await?;
    ...
    if num_unspent_basic_outputs_with_send_amount < 127 {
        ...
        let transaction = wallet
            .send_with_params(
                &secret_manager,
                vec![SendParams::new(SEND_AMOUNT, recv_address.clone())?; 127],
                None,
            )
            .await?;
    ...
    for i in 1..=NUM_ROUNDS {
        ...
        for n in 0..num_simultaneous_txs {
            ...
            tasks.spawn(async move {
                ...
                let transaction = wallet
                    .send(&secret_manager, SEND_AMOUNT, recv_address, None)
                    .await
                    .map_err(|err| (n, err))?;
        ...
        if error_state.is_err() {
            ...
            let mut balance = wallet.sync(&secret_manager, None).await?;
            println!("Wallet synced");

            while balance.base_coin().available() == 0 {
                println!("No funds available");
                tokio::time::sleep(std::time::Duration::from_secs(2)).await;
                balance = wallet.sync(&secret_manager, None).await?;
                println!("Wallet synced");
            }
        }
