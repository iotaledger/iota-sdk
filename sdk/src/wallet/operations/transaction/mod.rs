...
impl Wallet {
    ...
    /// }
    /// ```
    pub async fn send_outputs<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        ...
        crate::client::Error: From<S::Error>,
    {
        ...
        self.finish_transaction(secret_manager, outputs, options).await
    }

    /// Separated function from send, so syncing isn't called recursively with the consolidation function, which sends
    /// transactions
    pub async fn finish_transaction<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        outputs: impl Into<Vec<Output>> + Send,
        options: impl Into<Option<TransactionOptions>> + Send,
        ...
        crate::client::Error: From<S::Error>,
    {
        ...
        self.sign_and_submit_transaction(secret_manager, prepared_transaction_data, options)
            .await
    }

    /// Signs a transaction, submit it to a node and store it in the wallet
    pub async fn sign_and_submit_transaction<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        ...
        crate::client::Error: From<S::Error>,
    {
        ...
        let signed_transaction_data = match self.sign_transaction(secret_manager, &prepared_transaction_data).await {
            Ok(res) => res,
            Err(err) => {
        ...
        self.submit_and_store_transaction(secret_manager, signed_transaction_data, options)
            .await
    }

    /// Validates the transaction, submit it to a node and store it in the wallet
    pub async fn submit_and_store_transaction<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        ...
        crate::client::Error: From<S::Error>,
    {
        ...
        let block_id = match self
            .submit_signed_transaction(secret_manager, signed_transaction_data.payload.clone())
            .await
        {
