...
impl Wallet {
    ...
    pub async fn sync<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        options: Option<SyncOptions>,
    ) -> crate::wallet::Result<Balance>
    where
        crate::client::Error: From<S::Error>,
    {
        ...
        if options.sync_pending_transactions {
            let confirmed_tx_with_unknown_output = self.sync_pending_transactions(secret_manager).await?;
