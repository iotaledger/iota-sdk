...
impl Wallet {
    ...
    /// transaction.
    pub async fn consolidate_outputs<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
    ...
    {
        let prepared_transaction = self.prepare_consolidate_outputs(secret_manager, params).await?;
        let consolidation_tx = self
            .sign_and_submit_transaction(secret_manager, prepared_transaction, None)
            .await?;
    ...
    pub async fn prepare_consolidate_outputs<S: 'static + SecretManage>(
        &self,
        secret_manager: &S,
        params: ConsolidationParams,
