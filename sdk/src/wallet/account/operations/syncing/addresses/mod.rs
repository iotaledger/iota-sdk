...
//         &self,
//         options: &SyncOptions,
//     ) -> crate::wallet::Result<Vec<AddressWithUnspentOutputs>> { log::debug!("[SYNC] get_addresses_to_sync");
...
//             // Add currently known unspent output ids, so we can later compare them with the new output ids and see
// if             // one got spent (is missing in the new returned output ids)
