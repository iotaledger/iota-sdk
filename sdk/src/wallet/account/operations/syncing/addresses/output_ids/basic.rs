    ...
    pub(crate) async fn get_basic_output_ids_with_any_unlock_condition(
        ...
        #[cfg(target_family = "wasm")]
        {
            ...
            output_ids.extend(
                self.client()
                    .basic_output_ids(
                        BasicOutputQueryParameters::new().storage_deposit_return_address(bech32_address.clone()),
                    )
                    .await?
                    .items,
            );
            ...
            let tasks = [
                ...
                async {
                    ...
                    tokio::spawn(async move {
                        client
                            .basic_output_ids(
                                BasicOutputQueryParameters::new().unlockable_by_address(bech32_address.clone()),
                            )
                            .await
                            .map_err(From::from)
                    })
                ...
                async {
                    ...
                    tokio::spawn(async move {
                        client
                            .basic_output_ids(
                                BasicOutputQueryParameters::new()
