...
use crate::{
    ...
    types::block::{
        ...
        output::{
            ...
            BasicOutputBuilder, MinimumStorageDeposit, NativeToken, NftId, NftOutput, NftOutputBuilder, Output, Rent,
        },
        Error,
    },
    ...
    crate::wallet::Error: From<S::Error>,
{
    ...
    pub async fn prepare_output(
            ...
            Ordering::Less => {
                ...
                if storage_deposit.return_strategy.unwrap_or_default() == ReturnStrategy::Return {
                    ...
                    let min_storage_deposit_return_amount =
                        MinimumStorageDeposit::new(rent_structure, token_supply).finish()?;
        ...
        if second_output.amount() < required_storage_deposit {
            ...
            let minimum_storage_deposit = MinimumStorageDeposit::new(rent_structure, token_supply).finish()?;
