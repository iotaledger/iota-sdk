...
fn input_signing_data_conversion() {
    ...
    let bip32_chain = Chain::from_u32_hardened([HD_WALLET_TYPE, SHIMMER_COIN_TYPE, 0, 0, 0]);
    ...
    let input_signing_data = InputSigningData {
        ...
        chain: Some(bip32_chain.clone()),
    };

    assert_eq!(input_signing_data.chain.as_ref(), Some(&bip32_chain));

    let input_signing_data_str = r#"{"output":{"type":3,"amount":"1000000","unlockConditions":[{"type":0,"address":{"type":0,"pubKeyHash":"0x7ffec9e1233204d9c6dce6812b1539ee96af691ca2e4d9065daa85907d33e5d3"}}]},"outputMetadata":{"blockId":"0xedf5f572c58ddf4b4f9567d82bf96689cc68b730df796d822b4b9fb643f5efda","transactionId":"0xbce525324af12eda02bf7927e92cea3a8e8322d0f41966271443e6c3b245a440","outputIndex":0,"isSpent":false,"milestoneIndexBooked":0,"milestoneTimestampBooked":0,"ledgerIndex":0},"chain":[44,4219,0,0,0]}"#;
    assert_eq!(
        serde_json::to_string(&input_signing_data).unwrap(),
        input_signing_data_str
    );

    let restored_input_signing_data = serde_json::from_str::<InputSigningData>(input_signing_data_str).unwrap();
    assert_eq!(restored_input_signing_data.chain.as_ref(), Some(&bip32_chain));

    assert!(restored_input_signing_data.output.is_basic());
    assert_eq!(restored_input_signing_data.chain, Some(bip32_chain));
}
