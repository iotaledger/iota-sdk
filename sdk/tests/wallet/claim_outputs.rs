...
async fn claim_2_basic_micro_outputs() -> Result<()> {
    ...
    let tx = accounts[1]
        .send_with_params(
            [
                SendParams::new(micro_amount, accounts[0].first_address_bech32().await.unwrap())?,
                SendParams::new(micro_amount, accounts[0].first_address_bech32().await.unwrap())?,
            ],
            TransactionOptions {
...
async fn claim_1_of_2_basic_outputs() -> Result<()> {
    ...
    let tx = accounts[1]
        .send_with_params(
            [
                SendParams::new(amount, accounts[0].first_address_bech32().await.unwrap())?,
                SendParams::new(0, accounts[0].first_address_bech32().await.unwrap())?,
            ],
            TransactionOptions {
                allow_micro_amount: true,
                ..Default::default()
        ...
        .send_native_tokens(
            [
                SendNativeTokensParams::new(
                    accounts[0].first_address_bech32().await.unwrap(),
