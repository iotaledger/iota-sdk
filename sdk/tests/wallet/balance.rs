...
async fn addresses_balance() -> Result<()> {
    ...
    let balance_1: Balance = account_1.addresses_balance(vec![acc_1_addr.address()]).await?;
    ...
    let balance_1: Balance = account_1.addresses_balance(vec![acc_1_addr_2.address()]).await?;
