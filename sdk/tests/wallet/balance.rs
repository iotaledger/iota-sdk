...
async fn balance_expiration() -> Result<()> {
    ...
    let (wallet_2, secret_manager_2) = make_wallet(storage_path_2, None, None).await?;
    ...
    let balance = wallet_2.sync(&secret_manager_2, None).await?;
    assert_eq!(balance.potentially_locked_outputs().len(), 1);
    assert_eq!(balance.base_coin().total(), 0);
    assert_eq!(balance.base_coin().available(), 0);
    ...
    let outputs = [BasicOutputBuilder::new_with_amount(1_000_000)
        ...
        .finish_output()?];
    let _tx = wallet_2.send_outputs(&secret_manager_2, outputs, None).await?;
...
async fn balance_transfer() -> Result<()> {
    ...
    let balance_0_sync = wallet_0.sync(&secret_manager_0, None).await?;
    ...
    let balance_1 = wallet_1.sync(&secret_manager_1, None).await?;
    assert_eq!(balance_1.base_coin().available(), 0);

    // Send to 1
    let tx = wallet_0
        .send(&secret_manager_0, to_send, wallet_1.address().await, None)
        .await?;
    ...
    let balance_1_sync = wallet_1.sync(&secret_manager_1, None).await?;
...
async fn balance_voting_power() -> Result<()> {
    ...
    let tx = wallet.increase_voting_power(&secret_manager, voting_power).await?;
    wallet
    ...
    let tx = wallet
        .increase_voting_power(&secret_manager, faucet_amount - voting_power)
        .await?;
    wallet
        .reissue_transaction_until_included(&secret_manager, &tx.transaction_id, None, None)
        .await?;
