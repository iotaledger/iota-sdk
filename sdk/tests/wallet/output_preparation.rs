...
async fn output_preparation() -> Result<()> {
    ...
    let output = account
        ...
        .await?;
    let rent_struct = wallet.client().get_rent_parameters().await?.into();
...
async fn prepare_output_remainder_dust() -> Result<()> {
    ...
    let minimum_required_storage_deposit = MinimumStorageDepositBasicOutput::new(rent_struct, token_supply).finish()?;
...
async fn prepare_existing_nft_output_gift() -> Result<()> {
    ...
    let rent_struct = wallet.client().get_rent_parameters().await?.into();
    let minimum_storage_deposit = Output::Nft(nft.clone()).rent_cost(rent_struct);
