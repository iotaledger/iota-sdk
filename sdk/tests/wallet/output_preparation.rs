...
use iota_sdk::{
    types::block::{
        ...
        output::{NativeToken, NftId, TokenId, MinimumStorageDepositBasicOutput},
    },
    wallet::{
...
async fn prepare_output_remainder_dust() -> Result<()> {
    ...
    // Check if the output has enough amount to cover the storage deposit
    output.verify_storage_deposit(rent_structure, token_supply)?;
    // The left over 21299 is too small to keep, so we donate it, but the deposit we return.
    // WARN: We end up sending less to recipient due to most returning from the deposit
    assert_eq!(output.amount(), balance.base_coin().available());
    // storage deposit gifted, only address unlock condition
    assert_eq!(output.unlock_conditions().unwrap().len(), 2);

    account.send_outputs(vec![output], None).await?;
    ...
    // Check if the output has enough amount to cover the storage deposit
    output.verify_storage_deposit(rent_structure, token_supply)?;
    // We use excess if leftover is too small, so amount is equal to min storage deposit
    // WARN: What happens to the leftover 21300 on our address??
    assert_eq!(output.amount(), 42600);
    // storage deposit gifted, only address unlock condition
    assert_eq!(output.unlock_conditions().unwrap().len(), 1);
    ...
    // Check if the output has enough amount to cover the storage deposit
    output.verify_storage_deposit(rent_structure, token_supply)?;
    // We use a storage deposit to allow smaller amount
    assert_eq!(output.amount(), 46800);
