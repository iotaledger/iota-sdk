...
pub(crate) async fn make_wallet(
    ...
    Ok((wallet_builder.finish(&secret_manager).await?, secret_manager))
}
...
#[allow(dead_code)]
pub(crate) async fn request_funds<S: 'static + SecretManage>(wallet: &Wallet, secret_manager: &S) -> Result<()>
where
