...
use iota_sdk::types::{
    block::{
        ...
        output::{AccountOutput, Feature, FoundryId, NativeToken, Output, SimpleTokenScheme, TokenId},
...
fn builder() {
    ...
    assert_eq!(
        output.amount(),
        Output::Account(output.clone()).rent_cost(protocol_parameters.rent_parameters().into())
    );
