...
pub(crate) async fn call_wallet_method_internal<S: 'static + SecretManage + Clone>(
    wallet: &Wallet,
    secret_manager: &S,
    method: WalletMethod,
    ...
    iota_sdk::client::Error: From<S::Error>,
{
    let response = match method {
        ...
        WalletMethod::Backup { destination, password } => {
            wallet.backup(secret_manager, destination, password).await?;
            Response::Ok
        }
        // #[cfg(feature = "stronghold")]
        ...
        WalletMethod::GenerateEd25519Address {
            ...
            let address = wallet
                .generate_ed25519_address(secret_manager, account_index, address_index, options)
                .await?;
        ...
        WalletMethod::StartBackgroundSync {
            ...
            let duration = interval_in_milliseconds.map(Duration::from_millis);
            wallet
                .start_background_syncing(secret_manager, options, duration)
                .await?;
            Response::Ok
        }
        ...
        WalletMethod::ClaimOutputs { output_ids_to_claim } => {
            let transaction = wallet
                .claim_outputs(secret_manager, output_ids_to_claim.to_vec())
                .await?;
        ...
        WalletMethod::PrepareImplicitAccountTransition { output_id } => {
            let data = wallet
                .prepare_implicit_account_transition(secret_manager, &output_id)
                .await?;
        ...
        WalletMethod::ReissueTransactionUntilIncluded {
            ...
            let block_id = wallet
                .reissue_transaction_until_included(secret_manager, &transaction_id, interval, max_attempts)
                .await?;
        ...
        WalletMethod::Send {
            ...
            let transaction = wallet.send(secret_manager, amount, address, options).await?;
            Response::SentTransaction(TransactionWithMetadataDto::from(&transaction))
        }
        WalletMethod::SendWithParams { params, options } => {
            let transaction = wallet.send_with_params(secret_manager, params, options).await?;
            Response::SentTransaction(TransactionWithMetadataDto::from(&transaction))
        }
        ...
        WalletMethod::Sync { options } => Response::Balance(wallet.sync(secret_manager, options).await?),
