...
pub(crate) fn call_utils_method_internal(method: UtilsMethod) -> Result<Response> {
    let response = match method {
        ...
        UtilsMethod::HexToBech32 { hex, bech32_hrp } => {
            Response::Bech32Address(hex_to_bech32(&hex, bech32_hrp)?.into())
        }
        UtilsMethod::AccountIdToBech32 { account_id, bech32_hrp } => {
            Response::Bech32Address(account_id.to_bech32(bech32_hrp).into())
        }
        UtilsMethod::NftIdToBech32 { nft_id, bech32_hrp } => {
            Response::Bech32Address(nft_id.to_bech32(bech32_hrp).into())
        }
        UtilsMethod::HexPublicKeyToBech32Address { hex, bech32_hrp } => {
            Response::Bech32Address(hex_public_key_to_bech32_address(&hex, bech32_hrp)?.into())
        }
