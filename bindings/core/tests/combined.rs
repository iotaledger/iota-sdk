...
use iota_sdk_bindings_core::{CallMethod, Response, Result, WalletMethod, WalletOperationMethod, WalletOptions};
...
async fn create_wallet() -> Result<()> {
    ...
    let response = wallet
        .call_method(WalletMethod::CallMethod {
            method: WalletOperationMethod::UnspentOutputs { filter_options: None },
        })
        .await;
...
// TODO: temporarily disabled (needs to become `verify_wallet`)
// #[tokio::test]
// async fn verify_accounts() -> Result<()> {
//     let storage_path = "test-storage/verify_accounts";
//     std::fs::remove_dir_all(storage_path).ok();

//     let secret_manager = r#"{"Mnemonic":"about solution utility exist rail budget vacuum major survey clerk pave
// ankle wealth gym gossip still medal expect strong rely amazing inspire lazy lunar"}"#;     let client_options = r#"{
//             "nodes":[
//                {
//                   "url":"http://localhost:14265",
//                   "auth":null,
