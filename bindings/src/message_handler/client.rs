...
use crate::{message::ClientMessage, message_handler::Result, response::Response};
...
pub async fn call_client_method_internal(client: &Client, message: ClientMessage) -> Result<Response> {
    match message {
        ...
        ClientMessage::GetNode => Ok(Response::Node(client.get_node()?)),
        ClientMessage::GetNetworkInfo => Ok(Response::NetworkInfo(client.get_network_info().await?.into())),
        ClientMessage::GetNetworkId => Ok(Response::NetworkId(client.get_network_id().await?)),
        ClientMessage::GetBech32Hrp => Ok(Response::Bech32Hrp(client.get_bech32_hrp().await?)),
        ClientMessage::GetMinPowScore => Ok(Response::MinPowScore(client.get_min_pow_score().await?)),
        ClientMessage::GetTipsInterval => Ok(Response::TipsInterval(client.get_tips_interval())),
        ...
        ClientMessage::GetLocalPow => Ok(Response::Bool(client.get_local_pow())),
        ClientMessage::GetFallbackToLocalPow => Ok(Response::Bool(client.get_fallback_to_local_pow())),
        #[cfg(feature = "ledger_nano")]
        ...
        ClientMessage::VerifyEd25519Signature {
            signature,
            message,
            address,
        } => {
        ...
        ClientMessage::GetHealth { url } => Ok(Response::Bool(client.get_health(&url).await?)),
        ClientMessage::GetNodeInfo { url, auth } => Ok(Response::NodeInfo(Client::get_node_info(&url, auth).await?)),
        ClientMessage::GetInfo => Ok(Response::Info(client.get_info().await?)),
        ClientMessage::GetPeers => Ok(Response::Peers(client.get_peers().await?)),
        ClientMessage::GetTips => Ok(Response::Tips(client.get_tips().await?)),
        ...
        ClientMessage::GetBlock { block_id } => {
            Ok(Response::Block(BlockDto::from(&client.get_block(&block_id).await?)))
        }
        ClientMessage::GetBlockMetadata { block_id } => {
            Ok(Response::BlockMetadata(client.get_block_metadata(&block_id).await?))
        }
        ClientMessage::GetBlockRaw { block_id } => Ok(Response::BlockRaw(client.get_block_raw(&block_id).await?)),
        ...
        ClientMessage::GetReceipts => Ok(Response::Receipts(client.get_receipts().await?)),
        ...
        ClientMessage::GetTreasury => Ok(Response::Treasury(client.get_treasury().await?)),
        ...
        ClientMessage::AliasOutputId { alias_id } => Ok(Response::OutputId(client.alias_output_id(alias_id).await?)),
        ClientMessage::NftOutputIds { query_parameters } => Ok(Response::OutputIdsResponse(
            client.nft_output_ids(query_parameters).await?,
        )),
        ClientMessage::NftOutputId { nft_id } => Ok(Response::OutputId(client.nft_output_id(nft_id).await?)),
        ...
        ClientMessage::GetOutputs { output_ids } => Ok(Response::Outputs(client.get_outputs(output_ids).await?)),
        ClientMessage::TryGetOutputs { output_ids } => Ok(Response::Outputs(client.try_get_outputs(output_ids).await?)),
        ...
        ClientMessage::IsAddressValid { address } => Ok(Response::Bool(Address::is_valid_bech32(&address))),
        ...
        ClientMessage::HashTransactionEssence { essence } => Ok(Response::TransactionEssenceHash(prefix_hex::encode(
