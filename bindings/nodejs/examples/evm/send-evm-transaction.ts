...
async function run() {
    ...
    try {
        ...
        const { signature } = await secretManager.signSecp256k1Ecdsa(messageToSign, bip44Path)
        ...
        strictEqual(senderAddress, signedTransaction.getSenderAddress().toString(), 'Mismatch in addresses', )   
    } catch (error) {
...
function createSignedTransaction(transaction: Transaction, signature: any): Transaction {
