...
pub type WalletMethodHandler = Arc<RwLock<Option<(Wallet, SecretManagerMethodHandler)>>>;

#[napi(js_name = "createWallet")]
pub async fn create_wallet(
    options: String,
    secret_manager: External<SecretManagerMethodHandler>,
) -> Result<External<WalletMethodHandler>> {
    let wallet_options = serde_json::from_str::<WalletOptions>(&options).map_err(NodejsError::from)?;
    let wallet = wallet_options
        .build(secret_manager.as_ref())
        .await
        .map_err(NodejsError::from)?;

    Ok(External::new(Arc::new(RwLock::new(Some((
        wallet,
        secret_manager.as_ref().clone(),
    ))))))
}
...
pub async fn call_wallet_method(wallet: External<WalletMethodHandler>, method: String) -> Result<String> {
    ...
    if let Some((wallet, secret_manager)) = &*wallet.as_ref().read().await {
        let res = rust_call_wallet_method(wallet, secret_manager, wallet_method).await;
...
pub async fn listen_wallet(
    ...
    if let Some((wallet, _)) = &*wallet.as_ref().read().await {
        wallet
...
pub async fn get_secret_manager(wallet: External<WalletMethodHandler>) -> Result<External<SecretManagerMethodHandler>> {
    if let Some((_, secret_manager)) = &*wallet.as_ref().read().await {
        Ok(External::new(secret_manager.clone()))
    } else {
        Err(Error::new(
            Status::GenericFailure,
            serde_json::to_string(&Response::Panic("Wallet got destroyed".to_string())).map_err(NodejsError::from)?,
