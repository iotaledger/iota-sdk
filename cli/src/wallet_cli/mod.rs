...
// `burn-nft` command
pub async fn burn_nft_command(wallet: &Wallet, secret_manager: &SecretManager, nft_id: String) -> Result<(), Error> {
    println_log_info!("Burning nft {nft_id}.");

    let transaction = wallet.burn(secret_manager, NftId::from_str(&nft_id)?, None).await?;
...
pub async fn claim_command(
    ...
    if let Some(output_id) = output_id {
        ...
        let transaction = wallet
            .claim_outputs(secret_manager, [OutputId::from_str(&output_id)?])
            .await?;
        ...
        for output_ids_chunk in output_ids.chunks(60) {
            let transaction = wallet.claim_outputs(secret_manager, output_ids_chunk.to_vec()).await?;
            println_log_info!(
                "Claiming transaction sent:\n{:?}\n{:?}",
                transaction.transaction_id,
                transaction.block_id
...
// `consolidate` command
pub async fn consolidate_command(wallet: &Wallet, secret_manager: &SecretManager) -> Result<(), Error> {
...
pub async fn create_account_output_command(wallet: &Wallet, secret_manager: &SecretManager) -> Result<(), Error> {
...
pub async fn create_native_token_command(
    ...
    let create_transaction = wallet.create_native_token(secret_manager, params, None).await?;
...
pub async fn destroy_account_command(
    wallet: &Wallet,
    secret_manager: &SecretManager,
    account_id: String,
) -> Result<(), Error> {
    ...
    let transaction = wallet
        .burn(secret_manager, AccountId::from_str(&account_id)?, None)
        .await?;
...
pub async fn destroy_foundry_command(
    wallet: &Wallet,
    secret_manager: &SecretManager,
    foundry_id: String,
) -> Result<(), Error> {
    ...
    let transaction = wallet
        .burn(secret_manager, FoundryId::from_str(&foundry_id)?, None)
        .await?;
...
pub async fn implicit_account_transition_command(
    ...
    let transaction = wallet.implicit_account_transition(secret_manager, &output_id).await?;
...
pub async fn mint_nft_command(
    ...
    let nft_options = MintNftParams::new()
        ...
        .with_sender(sender)
        .with_issuer(issuer);
    let transaction = wallet.mint_nfts(secret_manager, [nft_options], None).await?;
...
pub async fn send_native_token_command(
    ...
    let transaction = if gift_storage_deposit.unwrap_or(false) {
        ...
        wallet.send_outputs(secret_manager, outputs, None).await?
    } else {
        ...
        wallet.send_native_tokens(secret_manager, outputs, None).await?
    };
...
// `sync` command
pub async fn sync_command(wallet: &Wallet, secret_manager: &SecretManager) -> Result<(), Error> {
    let balance = wallet
        .sync(
            secret_manager,
            Some(SyncOptions {
...
pub async fn vote_command(
    ...
    let transaction = wallet.vote(secret_manager, Some(event_id), Some(answers)).await?;
...
pub async fn increase_voting_power_command(
    wallet: &Wallet,
    secret_manager: &SecretManager,
    amount: u64,
) -> Result<(), Error> {
    let transaction = wallet.increase_voting_power(secret_manager, amount).await?;
...
pub async fn decrease_voting_power_command(
    ...
    let transaction = wallet.decrease_voting_power(secret_manager, amount).await?;
...
pub async fn prompt(wallet: &Wallet, secret_manager: &Arc<SecretManager>) -> Result<(), Error> {
    ...
    loop {
        match prompt_internal(wallet, secret_manager, &mut rl).await {
            Ok(res) => match res {
                ...
                _ => {
                    ...
                    match protocol_cli.command {
                        ...
                        WalletCommand::BurnNativeToken { token_id, amount } => {
                            burn_native_token_command(wallet, secret_manager, token_id, amount).await
                        }
                        WalletCommand::BurnNft { nft_id } => burn_nft_command(wallet, secret_manager, nft_id).await,
                        WalletCommand::Claim { output_id } => claim_command(wallet, secret_manager, output_id).await,
                        WalletCommand::ClaimableOutputs => claimable_outputs_command(wallet).await,
                        WalletCommand::Consolidate => consolidate_command(wallet, secret_manager).await,
                        WalletCommand::CreateAccountOutput => {
                            create_account_output_command(wallet, secret_manager).await
                        }
                        ...
                        WalletCommand::DestroyAccount { account_id } => {
                            destroy_account_command(wallet, secret_manager, account_id).await
                        }
                        WalletCommand::DestroyFoundry { foundry_id } => {
                            destroy_foundry_command(wallet, secret_manager, foundry_id).await
                        }
                        WalletCommand::Exit => {
                        ...
                        WalletCommand::ImplicitAccountTransition { output_id } => {
                            implicit_account_transition_command(wallet, secret_manager, output_id).await
                        }
                        WalletCommand::ImplicitAccounts => implicit_accounts_command(wallet).await,
                        WalletCommand::MeltNativeToken { token_id, amount } => {
                            melt_native_token_command(wallet, secret_manager, token_id, amount).await
                        }
                        WalletCommand::MintNativeToken { token_id, amount } => {
                            mint_native_token_command(wallet, secret_manager, token_id, amount).await
                        }
                        ...
                        WalletCommand::SendNft { address, nft_id } => {
                            send_nft_command(wallet, secret_manager, address, nft_id).await
                        }
                        WalletCommand::Sync => sync_command(wallet, secret_manager).await,
                        ...
                        WalletCommand::Vote { event_id, answers } => {
                            vote_command(wallet, secret_manager, event_id, answers).await
                        }
                        WalletCommand::StopParticipating { event_id } => {
                            stop_participating_command(wallet, secret_manager, event_id).await
                        }
                        ...
                        WalletCommand::IncreaseVotingPower { amount } => {
                            increase_voting_power_command(wallet, secret_manager, amount).await
                        }
                        WalletCommand::DecreaseVotingPower { amount } => {
                            decrease_voting_power_command(wallet, secret_manager, amount).await
                        }
