...
pub async fn new_wallet(cli: Cli) -> Result<Option<(Wallet, Arc<SecretManager>)>, Error> {
    ...
    Ok(if let Some(command) = cli.command {
        match command {
            ...
            CliCommand::ChangePassword => {
                let res = change_password_command(storage_path, snapshot_path).await?;
                Some(res)
            }
            CliCommand::Init(init_parameters) => {
                let res = init_command(storage_path, snapshot_path, init_parameters).await?;
                Some(res)
            }
            ...
            CliCommand::Restore { backup_path } => {
                let res = restore_command(storage_path, snapshot_path, std::path::Path::new(&backup_path)).await?;
                Some(res)
            }
            CliCommand::SetNodeUrl { url } => {
                let res = set_node_url_command(storage_path, snapshot_path, url).await?;
                Some(res)
            }
            CliCommand::Sync => {
                let res = sync_command(storage_path, snapshot_path).await?;
                Some(res)
            }
        }
    } else {
...
pub async fn backup_command(storage_path: &Path, snapshot_path: &Path, backup_path: &Path) -> Result<(), Error> {
    ...
    let (wallet, secret_manager) = unlock_wallet(storage_path, snapshot_path, password.clone()).await?;
    wallet.backup(&secret_manager, backup_path.into(), password).await?;
...
pub async fn change_password_command(
    ...
    if let SecretManager::Stronghold(s) = secret_manager.as_ref() {
        s.change_stronghold_password(password, new_password).await?;
    }

    println_log_info!("The password has been changed");

    Ok((wallet, secret_manager))
}
...
pub async fn init_command(
    ...
    let secret_manager = Arc::new(SecretManager::Stronghold(secret_manager));
...
pub async fn node_info_command(storage_path: &Path) -> Result<(Wallet, Arc<SecretManager>), Error> {
    let (wallet, secret_manager) = unlock_wallet(storage_path, None, None).await?;
...
pub async fn restore_command(
    ...
    let secret_manager = if check_file_exists(snapshot_path).await.is_ok() {
        println!(
    ...
    wallet
        .restore_backup(&secret_manager, backup_path.into(), password, None, None)
        .await?;
...
pub async fn set_node_url_command(
    storage_path: &Path,
    snapshot_path: &Path,
    url: String,
) -> Result<(Wallet, Arc<SecretManager>), Error> {
    let password = get_password("Stronghold password", !snapshot_path.exists())?;
    let (wallet, secret_manager) = unlock_wallet(storage_path, snapshot_path, password).await?;
    wallet.set_client_options(ClientOptions::new().with_node(&url)?).await?;

    Ok((wallet, secret_manager))
}

pub async fn sync_command(storage_path: &Path, snapshot_path: &Path) -> Result<(Wallet, Arc<SecretManager>), Error> {
    let password = get_password("Stronghold password", !snapshot_path.exists())?;
    let (wallet, secret_manager) = unlock_wallet(storage_path, snapshot_path, password).await?;
    let total_balance = wallet.sync(&secret_manager, None).await?;
...
pub async fn unlock_wallet(
    ...
    let maybe_wallet = Wallet::builder()
        .with_storage_path(&storage_path.to_string_lossy())
        .finish(&secret_manager)
        .await;
    ...
    Ok((maybe_wallet?, secret_manager))
}
